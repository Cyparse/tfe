...
export default function Dashboard({ onNavigate }) {
  ...
  const loadRecentActivity = async () => {
    const [{ data: regs }, { data: tickets }] = await Promise.all([
      supabase
        .from("registrations")
        .select("id, first_name, last_name, type, created_at")
        .order("created_at", { ascending: false })
        .limit(5),
      supabase
        .from("ticket_orders")
        .select("id, first_name, last_name, ticket_count, created_at")
        .order("created_at", { ascending: false })
        .limit(5),
    ]);
    const items = [
    ...
    <div className="space-y-10">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          />
          <StatCard
            title="Participants amateurs"
            value={stats.registrations.amateur}
            accent="var(--color-light-blue)"
          />
          <StatCard
            title="Participants professionnels"
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            ...
            accent="var(--color-festival-yellow)"
          />
          <StatCard
        ...
        <section className="lg:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {quickActions.map((a) => (
              ...
              >
                <span
                  className={`material-symbols-outlined mb-2 block${a.loading ? " animate-spin" : ""}`}
